...
The sidecar's record IDs were cross-referenced with the PTB-XL subset among the public 672-record cohort. Of 378 PTB-XL records, 153 have at least one mapped pathology target, 224 are listed as having no target, and one does not appear in the map. This is label-coverage inventory only; it is not a performance evaluation, and checkpoint overlap with these records is not established. A read-only filename search under CEDIA `03_OUTPUTS` found no Primary-5 pathology prediction or metric artifact. The source map and checkpoint were not copied into this repository. Sanitized hashes, coverage counts, and the explicit evidence boundary are in [cedia-pathology-crosscheck.json](cedia-pathology-crosscheck.json).
